...
var db = { me:{}, clients:[], bank:'', note:'', logo:'',
           noPrefix:'', noDigits:3, usedNos:[], lastSeq:0 };
...
function load(){
  try{
    ...
    if(o && typeof o === 'object'){
      ...
      db.usedNos  = o.usedNos || [];
      db.lastSeq  = o.lastSeq || 0;
      return true;
    }
  }catch(e){}
  return false;
...
function headHtml(total){
  ...
  h += '<p style="margin:10px 0 0">下記のとおりご請求申し上げます。</p>';

  h += '<div class="inv-total-box"><span class="lbl">ご請求金額（税込）</span>' +
...
function footHtml(sub, base, tax10, tax8, total){
  ...
  var pay = [];
  if($('dueDate').value) pay.push('お支払い期限　' + jpDate($('dueDate').value));
  if($('bank').value.trim()) pay.push($('bank').value.trim());
  if(pay.length){
    h += '<div class="inv-pay"><span class="ttl">お支払いについて</span>' +
...
function contHtml(){
  ...
  h += '<span>請求書' + (no ? '　No. ' + esc(no) : '') + '（続き）</span>';
...
function sampleAdd(kind){
  ...
  if(!$('bank').value.trim() && !db.bank){
    $('bank').value = 'さくら銀行　丸の内支店\n普通　1234567\nヤマダ ハナコ';
  }
  if(!$('note').value.trim() && !db.note){
    $('note').value = 'お振込手数料は貴社にてご負担くださいますようお願いいたします。';
  }
...
function boot(){
  ...
  if(db.bank) $('bank').value = db.bank;
  if(db.note) $('note').value = db.note;

  drawClients();

  /* ⭐一度決めたところは、たたんでおく（2026-09-09 本人「一回限り」「スクロールが長い」）
     ⚠③（請求の内容）は毎回使うので、いつも開いたまま。
     ⚠はじめての人は全部開いた状態＝何をする道具か分かるように */
  if(hasSavedMe()) $('meBox').open = false;
  if(db.clients.length) $('s2Box').open = false;
  if(db.bank || db.note) $('s4Box').open = false;
  ...
  Array.prototype.forEach.call(sels, function(sel){
    sel.onchange = function(){
      ...
      if(this.closest('#sampleTop')){
        ...
        var y = $('sampleLow').getBoundingClientRect().top + window.pageYOffset - 52;
  ...
  /* --- 支払い --- */
  $('dueEnd').onclick = function(){ $('dueDate').value = nextMonthEnd(); calc(); };
