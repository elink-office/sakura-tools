...
(function () {
  ...
  function drawSheet() {
    ...
    var g = $('grid');
    /* 🔴⭐minmax(0,1fr) にする（2026-09-10）。
       ⚠1fr は「中身がこれ以上は縮められない幅」より小さくならない。
    ...
    /* ⭐紙で中身に使える高さ。紙は 294mm（よこ 207mm）で、上下の padding が 12mm ずつ。
       ⚠紙の高さを 297 / 210 いっぱいにすると、わずかなはみ出しで全体が縮む */
    var pageH = (wide ? 207 : 294) - 24;
    /* ⭐表いがいが使う高さ。2026-09-10 に画面で実測した値をそのまま入れている。
       ⚠前は「念のための余裕 8mm」を足していたので、そのぶん表が短くなり、
         紙の下が空いていた（本人「下の余白がいっぱいあるんだよ」）*/
    var reserve = 10                              // 見出しと日付（下のあきこみ 9.7mm）
      + (frontWord() ? 16 : 0)                    // 前方の帯（上下のあきこみ 15.8mm）
      + ($('showCredit').checked ? 10 : 0);       // 右下のサイト名（9.4mm）
    var gapMM = 1.6 * (rows - 1);                 // マスとマスのすきま（6px）
    /* ⭐紙で使える高さ。1mm だけ余らせる（端数で2ページになるのを防ぐ）*/
    var limitH = pageH - 1;
    /* ⭐1マスの上限。⚠前は 60mm で頭打ちだったので、行が少ないと紙の下が大きく空いた
         （2026-09-10 本人「高さ、用紙にいっぱいにしてみて！」）*/
    var cap = 90;
    var mm = Math.max(10, Math.min(cap, (limitH - reserve - gapMM) / rows));
  ...
  function paperMM() {
    ...
    /* ⭐紙の外形（余白 12mm をふくむ）。中の余白は .sheet の padding で作る */
    return wide ? { w: 297, h: 207 } : { w: 210, h: 294 };
  }
  ...
  function showSample() {
    ...
    if (state.sample) {
      ...
      /* ⭐いつでも「紙の入れ物」のすぐ下に置き直す（2026-09-11 本人）。
         ⚠入れ物（#sheetBox）の中に入れると、入れ物の高さは紙のぶんしかないので、
           案内が下にはみ出して次の章に重なる */
      var box = $('sheetBox');
      if (box && note.previousElementSibling !== box) {
  ...
  function setPaper() {
    ...
    /* ⭐余白は紙の padding で作るので、ここは 0（print.css と合わせる）*/
    st.textContent = '@page{size:A4 ' + $('paper').value + ';margin:0}';
  ...
  function liftSheet() {
    ...
    /* 🔴⭐画面用の大きさを、印刷の前に自分ではがす（2026-09-10）。
       ⚠画面では紙を縮めて見せている（たての紙は 0.68倍ほど）。
         その縮小が紙に残ると、印刷まで小さくなる。
       ⚠CSS の transform:none だけに頼ると、効いていないことがある。
         ここで直接消せば、優先順位の争いに巻きこまれない。
       ⭐印刷が終わったら元に戻す（dropSheet）*/
    keepStyle = {
    ...
    el.style.removeProperty('--pxmm');
    /* 🔴⭐紙の大きさを、ここで直接書きこむ（2026-09-11）。
       ⚠CSS に書いた width / height は、ほかの指定に負けて効かないことがあった。
         その結果、紙が中身なりの大きさ（画面の幅ぶん）に広がり、
         用紙に収めるためブラウザが縮めていた。
         実測：たて 0.839倍、よこ 0.924倍（100mm の線が 83.9mm / 92.4mm で刷られていた）。
       ⭐ここで入れる指定は、CSS のどれよりも強い。
       ⭐高さは 3mm 余らせる（ぎりぎりだと、わずかなはみ出しで縮む）*/
    var wideP = $('paper') && $('paper').value === 'landscape';
    el.style.width = (wideP ? 297 : 210) + 'mm';
  ...
  function init() {
    ...
    function loadSample(kind) {
      ...
      if (kind === 2) {
        ...
        $('clsFree').value = '全学共通科目 情報リテラシー 期末試験';
        $('honor').value = '';
